...
import { IDENTIFICADOR_DE_PESQUISA_FRANQUIA, ITENS_FRANQUIA_SUBMENU } from './franquia.constant';
import {
  FranquiaEspecialidade,
  EChavePix,
  EEspecialidade,
  ETipoStatusFranquia,
  ETipoStatusFranquiaFerias,
} from './franquia.model';
...
export class FranquiaBuilder {
  ...
  private _definirSubmenu(model: CnCrudModel) {
    model.setSubmenu(ITENS_FRANQUIA_SUBMENU);
  }
    ...
    return new CnBaseDetalheModel(
      ...
      [
        new CnSessaoGrupoCamposDetalhe(FORM_TITULO_GENERICO, [
          CnGrupoCampoDetalhe.obterComoEntityUnica('Informações do franquia', [
        ...
        new CnSessaoGrupoCamposDetalhe('Informação de acesso', [
          CnGrupoCampoDetalhe.obterComoEntityUnica('Dados Bancários', [
            ...
            new CnCampoDetalhe('acesso.franquiaStatusTxt', 'Status'),
          ]),
        ]),
  ...
  private _gerarAcessoEtapa4(): CnStepperFormItemModel {
    return new CnStepperFormItemModel('acesso', 'Informações de acesso', [
      new CnGrupoCamposFormulario('', [
        ...
        CnInputCvaModel.obterCombobox(this._displayName.franquiaStatus.nomePropriedade, this._displayName.franquiaStatus.valorDisplay, true, FranquiaBuilder.obterOpcoesCampoStatusFranquia())
      ])
    ])
  }
  ...
  private _gerarInformacaoGeraisEtapa1(): CnStepperFormItemModel {
    return new CnStepperFormItemModel('franquia', FORM_TITULO_GENERICO, [
      new CnGrupoCamposFormulario('Informações do franquia', [
        ...
        CnInputCvaModel.obterComboboxMultiSelect(this._displayName.especialidades.nomePropriedade, this._displayName.especialidades.valorDisplay, true, FranquiaBuilder.obterOpcoesEspecialidades())
  ...
  private _exportarEspecilidade(): (valorControl: any) => ICnInputCvaValorObtido {
    return (valorControl: any[]): ICnInputCvaValorObtido => {
      const valorRetornar: FranquiaEspecialidade[] = [];
      valorControl.forEach(valorEspecialidade => {
        valorRetornar.push({
          franquiaId: GUID_VAZIO,
  ...
  private _importarEspecialidade(): (valorSetarNoControl: any) => ICnInputCvaValorImbutir {
    return (valorSetarNoControl: FranquiaEspecialidade[]): ICnInputCvaValorImbutir => {
      return {
  ...
  private _gerarDadosBancariosEtapa3(): CnStepperFormItemModel {
    ...
    return new CnStepperFormItemModel('dadosBancarios', 'Dados bancários', [
      ...
      new CnGrupoCamposFormulario('Pix', [
        CnInputCvaModel.obterCombobox(this._displayName.tipoChavePix.nomePropriedade, this._displayName.tipoChavePix.valorDisplay, true, FranquiaBuilder.obterOpcoesCampoTipoChavePix())
  ...
  private _gerarItensListagem = (): CnItemListagemExibicao[] => {
    return [
      new CnItemListagemExibicao(this._displayName.franquiaStatus.nomePropriedadeTxt(), this._displayName.franquiaStatus.valorDisplay, this._definirCssStatusListagem()),
  ...
  private _definirCssStatusListagem(): ((itemListagem: any) => string) {
    return (entity: any) => {
      switch (entity.franquiaStatus) {
        case ETipoStatusFranquia.EmFerias:
          return 'listagem-em-ferias-item';
        case ETipoStatusFranquia.Ativo:
          return 'listagem-ativo-item';
        case ETipoStatusFranquia.Inativo:
          return 'listagem-inativo-item';
        default:
          return 'erro-' + entity.franquiaStatus;
      }
    }
  }
  private _gerarPesquisa(): CnPesquisaModel {
    return CnPesquisaModel.ObterPesquisaModel(this._service.buscarAvancado, [
      ...
      CnInputCvaModel.obterCombobox(this._displayName.franquiaStatus.nomePropriedade, this._displayName.franquiaStatus.valorDisplay, false, [{ id: '', nome: 'Todos' }, ...FranquiaBuilder.obterOpcoesCampoStatusFranquia()]).setarClassTamanho(TAMANHO_RESPONSIVO_3),
      CnInputCvaModel.obterCombobox(this._displayName.especialidade.nomePropriedade, this._displayName.especialidade.valorDisplay, false, [{ id: '', nome: 'Todos' }, ...FranquiaBuilder.obterOpcoesEspecialidades()]).setarClassTamanho(TAMANHO_RESPONSIVO_3)
    ], IDENTIFICADOR_DE_PESQUISA_FRANQUIA);
  }

  static obterOpcoesCampoStatusFranquia(): OpcaoCombobox[] {
    return [
      new OpcaoCombobox(ETipoStatusFranquia.Ativo, 'Ativo'),
      new OpcaoCombobox(ETipoStatusFranquia.EmFerias, 'Em férias'),
      new OpcaoCombobox(ETipoStatusFranquia.Inativo, 'Inativo'),
    ];
  }
  static obterOpcoesCampoStatusFranquiaFerias(): OpcaoCombobox[] {
    return [
      new OpcaoCombobox(ETipoStatusFranquiaFerias.Programada, 'Programada'),
      new OpcaoCombobox(ETipoStatusFranquiaFerias.EmFerias, 'Em férias'),
      new OpcaoCombobox(ETipoStatusFranquiaFerias.Cancelada, 'Cancelada'),
      new OpcaoCombobox(ETipoStatusFranquiaFerias.Realizada, 'Realizada'),
    ];
  }
  ...
  static obterOpcoesEspecialidades(): OpcaoCombobox[] {
    return [
      ...
      new OpcaoCombobox(EEspecialidade.franquiaEstetica, "Franquia Estética"),
      new OpcaoCombobox(EEspecialidade.esteticaFacial, "Estética Facial"),
    ];
  }

  private _gerarRotaIndex(): string {
    return LINK_ROUTES.franquia.inicio;
  }
}
